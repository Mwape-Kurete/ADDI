...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['event_id'])) {
        ...
        $sql_event = "UPDATE events SET approved = 1 WHERE event_id = ?";
        ...
        if ($stmt_event->execute()) {
            if ($stmt_event->affected_rows > 0) {
                ...
                echo "No event was updated. Please check the event ID.";
            }
        } else {
            echo "Error approving event: " . $stmt_event->error;
        }
    ...
    if (isset($_POST['ask_id'])) {
        ...
        $sql_ask = "UPDATE ask SET approved = 1 WHERE ask_id = ?";
        ...
        if ($stmt_ask->execute()) {
            if ($stmt_ask->affected_rows > 0) {
                echo "Ask approved successfully!";
            } else {
                echo "No ask was updated. Please check the ask ID.";
            }
        } else {
            echo "Error approving ask: " . $stmt_ask->error;
        }
